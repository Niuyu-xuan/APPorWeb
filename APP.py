...
if st.session_state.name.strip() and st.session_state.student_id.strip():
    ...
    with col_chart1:
        ...
        for bar in bars:
            h = bar.get_height()
            ax1.text(bar.get_x()+bar.get_width()/2., h+1, f"{h}", 
                     ha="center", va="bottom", fontsize=10, fontweight="bold", fontproperties=font_prop)
        
        ax1.set_ylim(0, 105)
        ax1.grid(axis='y', alpha=0.3)
        ax1.set_xticklabels(subjects, rotation=20, fontsize=9, fontproperties=font_prop)
        st.pyplot(fig1)
    ...
    with col_chart2:
        ...
        ax2.plot(angles, scores_plot, color="#ff6b6b", linewidth=2)
        ax2.fill(angles, scores_plot, color="#ff6b6b", alpha=0.3)
        ax2.set_xticks(angles[:-1])
        ax2.set_xticklabels(subjects, fontsize=10, fontproperties=font_prop)
        ax2.set_ylim(0, 100)
        ax2.set_title("成绩雷达分布", pad=20, fontproperties=font_prop)
        st.pyplot(fig2)
